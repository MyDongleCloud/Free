...
//Permissions: [ "_public_", "_dongle_", "_localnetwork_", "_groupadmin_", "_groupuser_", "admin", "user", ... ]
	...
	cJSON_ArrayForEach(permission, elPermissions) {
		...
		if (strcmp(permission->valuestring, "_public_") == 0) {
			...
			char sz[256];
			snprintf(sz, sizeof(sz), "\t\tAppModulePermission %s\n", permission->valuestring);
			fwrite(sz, strlen(sz), 1, pfM);
		}
		requireNb++;
	}
...
#\n\
#Relation between /usr/local/modules/mydonglecloud/modulesdefault.json [1] and /disk/admin/_config_/_modules_.json [2]\n\
